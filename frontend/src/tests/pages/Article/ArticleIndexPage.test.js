...
describe("ArticleIndexPage tests", () => {
  ...
  test("renders three articles correctly for regular user", async () => {
    ...
    expect(
      screen.getByText((text) => text.includes("Team 02 Frontend Fixtures")),
    ).toBeInTheDocument();

    const Url = screen.getByText("https://www.youtube.com/watch?v=u-jJ-ZWFpeE");
    expect(Url).toBeInTheDocument();

    expect(
      screen.getByText((text) =>
        text.includes("This explains step 3 of team02"),
      ),
    ).toBeInTheDocument();

    const email = screen.getByText("sjilla@gmail.com");
